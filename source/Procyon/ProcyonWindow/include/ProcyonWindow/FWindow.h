...
#ifndef PRWINDOW_FWINDOW_H
#define PRWINDOW_FWINDOW_H
...
namespace PrWindow {
    ...
    class FWindow {
    public:
        FWindow(const FWindow& p_other) = delete;
        FWindow(FWindow&& p_other) noexcept = delete;

		FWindow& operator=(FWindow p_other) = delete;
        ...
        */
        explicit FWindow(const Device::FDevice& p_device, const Settings::FWindowSettings& p_windowSettings);

        /**
        * Return the current position of the window
        */
		[[nodiscard]] std::pair<int16_t, int16_t> GetPosition() const;
        ...
        * @param p_glfwWindow
        */
		static FWindow* GetFWindowInstance(GLFWwindow* p_glfwWindow);
	...
	private:
		void CreateGlfwWindow(const Settings::FWindowSettings& p_windowSettings);
    ...
    private:
		/**
	     * \brief This map is used by callbacks to find a "Window" instance out of a "GLFWwindow" instnace
	     */
		static std::unordered_map<GLFWwindow*, FWindow*> m_windowMap;

        // GLFWwindow
        GLFWwindow* m_glfwWindow = nullptr;
        Device::FDevice m_device;
